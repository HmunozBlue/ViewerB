...
function getDropdownItems(eventData, isTouchEvent = false, availableTools) {
  ...
  // Estas etiquetas son las que estan cuando se usa longitud, angulo, anotacion y se hace click derecho
  if (
    isTouchEvent &&
    nearbyToolData &&
  ...
  if (nearbyToolData) {
    defaultDropdownItems.forEach(function (item) {
      ...
      if (item.actionType === 'Delete') {
        item.text = 'Borrar Medida'; //Delete measurement
      }

      if (item.actionType === 'setLabel') {
        item.text = 'Re Etiquetar'; //Relabel
      }

      if (item.actionType === 'setDescription') {
        item.text = `${
          nearbyToolData.tool.description ? 'Editar' : 'Agregar'
          } Descripcion`;
      }
